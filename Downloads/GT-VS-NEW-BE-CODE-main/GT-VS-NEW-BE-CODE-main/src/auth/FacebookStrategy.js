...
module.exports = function (passport) {
  ...
  if (!process.env.FB_CLIENT_ID || !process.env.FB_CLIENT_SECRET) {
    console.log('Facebook OAuth credentials not configured, skipping Facebook strategy registration');
    return;
  }
  passport.use(
      ...
      async (req, accessToken, refreshToken, profile, done) => {
        ...
        try {
          ...
          if (existingUser) {
            ...
            if (!existingUser.providers.includes('facebook')) {
              return done(
               'This email is already registered . Please log in using your email and password  or google',
                null
              );
            }

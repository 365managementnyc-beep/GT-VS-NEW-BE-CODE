        ...
        socket.on('send-message', async (data) => {
            try {
                ...
                if (data?.alert_result?.alert === "True") {
                    ...
                    console.log("Notification sent to admin:", findAdmin._id);

                    // Alert admin users that a user is attempting to chat incorrectly
                    sendNotification({
                        userId: findAdmin._id,
                        title: 'Alert',
                        message: `${senderData?.fullName} may be attempting to chat in an unauthorized way with ${receiverData?.fullName} in booking of service '${findBooking?.service?.title}'`,
                        type: 'message',
                ...
                if (receiverSocketId && data?.alert_result?.alert !== "True") {
                ...
                if (receiverId.toString() !== userId.toString() && addMessage.alert_result?.alert !== "True") {
                    if (receiverSocketId) {
                ...
                if (data?.reply_result?.answer?.length > 0) {
                    ...
                    const addMessageAi = await MessagesModel.create(messageBodyAi);
                    ...
                    const messageEmitBody = {
                        chatScreenBody: {
                            ...
                            chatType: chatDetails?.chatType,
                            receiverId: userId,
                            latestMessage: addMessageAi?.content ?? '',
                            latesMessageId: addMessageAi?._id,
                            latestMessageType: addMessageAi?.contentType ?? 'text',
                            contentDescriptionType: addMessageAi?.contentDescriptionType ?? 'text',
                            latestMessageSentAt: addMessageAi?.createdAt,
                            latestMessageTitle: addMessageAi?.contentTitle ?? '',
                            fileSize: addMessageAi?.fileSize ?? '',
                            latestMessageDescription: addMessageAi?.contentDescription ?? '',
                            unreadCount: unreadCount,

                        },
                        messageScreenBody: {
                            chatId,
                            messageId: addMessageAi?._id,
                            bookingId,
                            sender: {
                                _id: receiverId,
                                name: receiverData?.fullName,
                                profilePicture: receiverData?.profilePicture ?? defaultImage,
                            },
                            content: addMessageAi?.content,
                            latestMessageSentAt: addMessageAi?.createdAt,
                            contentTitle: addMessageAi?.contentTitle,
                            fileSize: addMessageAi?.fileSize ?? '',
                            contentDescription: addMessageAi?.contentDescription,
                            contentType: addMessageAi?.contentType,
                            contentDescriptionType: addMessageAi?.contentDescriptionType ?? 'text'
                        }
                    }


                    const messageDeliveryStatus = msgDeliveryStatus({ userId, chat: { lastMessage: addMessageAi } }) || {};
                    io.to(userId.toString()).emit('receive-message', {
                        ...messageEmitBody,
                        chatScreenBody: {
                            ...messageEmitBody.chatScreenBody,
...
const sendNotification = async ({ userId, title, message, type, fortype, permission: tab }) => {
    console.log('sendNotification called with:........................................................................1');
    try {
        console.log('sendNotification called with:........................................................................12');
        const io = getIO();
        ...
        if (user.role === 'admin' && fortype) {
            ...
            if (permissions.mobile === true) {
                if (permissions.admin === true) {
                    const notification = new Notification({ userId: user._id, title, message, type });
        ...
        console.log('Sending notification to user:', user._id, title, message, type);

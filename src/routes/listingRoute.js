...
router.route('/:id').delete(requireAuth, restrictTo(["vendor", "admin"]), logActionMiddleware('Delete service', 'ServiceListing'), deleteServiceListing).patch(requireAuth, restrictTo(["vendor", "admin"]),  logActionMiddleware('Update service', 'ServiceListing'), updateServiceListing).get(getServiceListing);
